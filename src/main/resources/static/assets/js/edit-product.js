...
function updateSelectedDisplayEdit(selectedIds, allItemsMap, initialItemsMap = null) {
    ...
    if (hiddenInputsContainer) hiddenInputsContainer.innerHTML = hiddenInputs.join("");
}
...
function bindEditPcForm(item) {
    ...
    const selectedIds = new Set((item.computerItems || []).map(ci => String(ci.id)));
    ...
    function updateComputerItemsByCategory(categoryId) {
        const category = window.categories.find(c => String(c.id) === String(categoryId));
...
function openEditModalFromList(button, type = 'computerItem') {
    ...
    if (type === 'pc') {
        ...
        document.getElementById("edit-category").value = item.category?.name || "";
    }
