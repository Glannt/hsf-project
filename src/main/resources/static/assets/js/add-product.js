const formTemplates = {
    pc: {
      ...
      <div class="form-group">
        ...
        <select name="category.id" class="form-control" required id="create-category">
          <option value="" disabled selected>-- Chọn loại --</option>
        </select>
      </div>

      <div class="form-group">
        <label for="pc-computer-items" class="font-weight-bold mb-2">Chọn linh kiện</label>
        <select name="pc-computer-items" id="pc-computer-items" class="form-control" multiple required size="8" style="min-height: 180px;"></select>
      ...
      <!-- Hidden input để gửi các computerItems đã chọn -->
      <div id="pc-selected-hidden-inputs"></div>

      <div class="form-group">
        <label>Giá</label>
        <input type="number" name="price" id="pc-price" class="form-control" readonly>
      </div>
...
function openCreateProductModal(type) {
    ...
    document.getElementById("modalCreateProductTitle").textContent = template.title;
    ...
    if (type === "pc") {
        ...
        catSelect.addEventListener("change", () => {
            ...
            const category = window.categories.find(c => String(c.id) === selectedCategoryId);
            ...
            itemSelect.innerHTML = newItems.map(ci => {
            ...
            // Cập nhật UI
            updateSelectedDisplay(selectedIds, allItemsMap, priceInput, priceDisplay, selectedList);
        });
...
function updateSelectedDisplay(selectedIds, allItemsMap, priceInput, priceDisplay, selectedList) {
    let total = 0;
    const selectedHTML = [];
    const hiddenInputs = [];
    ...
    selectedIds.forEach(id => {
        ...
        if (item) {
            ...
            selectedHTML.push(`<li class="list-group-item py-1">${item.name} (${Number(item.price).toLocaleString("vi-VN")} VND)</li>`);
            hiddenInputs.push(`<input type="hidden" name="computerItems" value="${id}" />`);
        }
    });
